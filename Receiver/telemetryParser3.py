...
class TelemetryParser:
    def __init__(self, config_file=configFile):
        self.lastGPSTime: datetime = datetime.now(timezone.utc)  # use this by default until the pi rtc goes out of sync (i.e the UPS fails). It can be resynced if reconnected to internet.
        self.timeFetched: uint32 = uint32(0)  # Time since time variables were last updated in seconds #round(time.time() * 1000). Using numpy to force unsigned and integer overflows are needed
        ...
        if not fileExists(config_file):
            ...
            sys.exit(5)
        # configData = open(config_file)
    
        # with open(config_file) as configData:
        logger.debug("Loading config (this process takes a while)")
        self.config: dict[int, dict] = dict()
        # with load_workbook(config_file, read_only=True, keep_vba=True, data_only=True) as configBook:
        configBook = load_workbook(config_file, read_only=True, keep_vba=True,
        ...
        for row in configSheet.iter_rows(min_row=2):
            ...
            self.config[row[configColumns["CAN_ID (dec)"] - 1].value] = rowAsDict
        configBook.close()
        # print config
        for address, packet_config in self.config.items():
            logger.info("{}\t{}".format(hex(address), packet_config))

    def __getTime(self, recievedMillis: uint32) -> datetime:
    ...
    def translateMsg(self, msgBytesAndTime: bytearray) -> tuple[
        ...
        # CRC check
        msgCRCStatus = self.__checkCRC(msgBytesAndTime)
        if not msgCRCStatus:
            logger.debug(
        ...
        msgTime = self.__getTime(recievedMillisTime)
        ...
        # Translate
        msgItem: str = self.__fromConfig("ItemCC")
        msgSource: str = self.__fromConfig("SourceCC")
        msgDLC = self.__fromConfig("DLC")
        msgData = struct.unpack(self.__fromConfig("struct unpack code"),
                                msgBytes[3:(3 + msgDLC)])
        msgBody: dict = {}
        dataIterator = 0
        for i in range(msgDLC):
            fieldValue = self.__fromConfig("BYTE_" + str(i) + "CC")
            if not (
        ...
        if canId == 24 and msgData[4] != 0:  # can id for GPS Time and Fix message (hex: 0x0F6)
            ...
            try:
                ...
                self.timeFetched = recievedMillisTime  # update when data was last fetched
                logger.debug(
                    "GPS time is now: " + self.lastGPSTime.strftime("%Y-%m-%d %H:%M:%S"))
            except ValueError:
