...
def recv_bytes(new_data):
    ...
    while ser.in_waiting and new_data == False:
...
def bytes2canmsg(can_bytes, can_timestamp):
...
def parse_msg(message):
    with open('/home/pi/telem/receiver/recv_conf.json') as json_file: # There is already a global version so just use that
...
#ser = serial.Serial('/dev/ttyUSB0', 115200)    # If it's not working, check this
...
with open('/home/pi/telem/receiver/recv_conf.json') as json_file:
